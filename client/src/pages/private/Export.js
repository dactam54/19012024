...
const Export = () => {
    ...
    try {
      if (selectedItems && selectedItems.length > 0) {
        ...
        exportData.push({
          ...
          "Dữ liệu": formData.shipper,
        });
        exportData.push({
          "Thông tin": "Người nhận",
          "Dữ liệu": formData.user,
        });
        exportData.push({ "Thông tin": "Ngày Xuất", "Dữ liệu": formData.date });
        exportData.push({ "Thông tin": "Diễn giải", "Dữ liệu": formData.note });
        selectedItems.forEach((item, index) => {
          const product = data?.rows.find(
  ...
  const tableCellStyle1 = {
    padding: "8px",
    textAlign: "left",
    position: !isPrintModalOpen ? "sticky" :"",
    top: "-1px",
    zIndex: 100,
    backgroundColor: "#ddd",
  };
