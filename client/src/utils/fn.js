...
export const renderStar = (number) => {
  ...
  for (let i = 0; i < number; i++)
    stars.push(<AiFillStar size={16} color="#f59e0b" />);
...
export const formatNumber = (number) => {
  const letter =
    number >= Math.pow(10, 9) ? "B" : number >= Math.pow(10, 6) ? "M" : "K";
  const n =
    number < Math.pow(10, 3) ? 1 : number < Math.pow(10, 6) ? 1000 : 1000000;
...
export const formatLocalTime = (timestamp) => {
  const localTime = new Date(timestamp).toLocaleDateString("vi-VN");
